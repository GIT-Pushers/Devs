...
describe("Hackathon Platform - Complete Test Suite", function () {
  let hackathonPlatformCore;
  let githubVerifier;
  ...
  beforeEach(async function () {
    ...
    const ParticipationNFT = await ethers.getContractFactory(
    ...
    const HackathonPlatformCore = await ethers.getContractFactory(
      "HackathonPlatformCore"
    );
    hackathonPlatformCore = await HackathonPlatformCore.deploy(
      treasury.address,
    ...
    await participationNFT.setHackathonPlatformCore(
      await hackathonPlatformCore.getAddress()
    );
  });

  describe("1. Deployment", function () {
    it("Should set the correct platform treasury", async function () {
      expect(await hackathonPlatformCore.platformTreasury()).to.equal(
        treasury.address
      );
    });

    it("Should set the correct GitHubVerifier", async function () {
      expect(await hackathonPlatformCore.githubVerifier()).to.equal(
        await githubVerifier.getAddress()
      );
    });

    it("Should set the correct ParticipationNFT", async function () {
      expect(await hackathonPlatformCore.participationNFT()).to.equal(
        await participationNFT.getAddress()
      );
    });

    it("Should initialize hackathonCount to 0", async function () {
      expect(await hackathonPlatformCore.hackathonCount()).to.equal(0);
    });
  });
  ...
  describe("2. GitHub Verification", function () {
    it("Should verify GitHub account with valid signature", async function () {
      ...
      await expect(
        githubVerifier
          .connect(user1)
          .verifyGitHub("123456", "testuser1", nonce, timestamp, signature)
      )
    ...
    it("Should reject duplicate GitHub ID", async function () {
      ...
      await githubVerifier
        .connect(user1)
        .verifyGitHub("123456", "testuser1", nonce1, timestamp1, signature1);
      ...
      await expect(
        githubVerifier
          .connect(user2)
          .verifyGitHub("123456", "testuser2", nonce2, timestamp2, signature2)
  ...
  describe("3. Hackathon Creation", function () {
    ...
    it("Should create a hackathon with correct parameters", async function () {
      ...
      await expect(
        ...
        .to.emit(hackathonPlatformCore, "HackathonCreated")
        .withArgs(0, organizer.address, CREATION_FEE);

      const hackathon = await hackathonPlatformCore.getHackathon(0);
    ...
    it("Should create voting token for hackathon", async function () {
      ...
      const tokenAddress = await hackathonPlatformCore.getVotingToken(0);
  ...
  describe("4. Sponsorship", function () {
    ...
    it("Should accept sponsorship during sponsorship phase", async function () {
      await expect(
        hackathonPlatformCore
          .connect(sponsor1)
          .sponsorHackathon(hackathonId, "ipfs://sponsor1", {
            value: SPONSOR_AMOUNT,
          })
      )
        .to.emit(hackathonPlatformCore, "SponsorshipReceived")
        .withArgs(hackathonId, sponsor1.address, SPONSOR_AMOUNT);

      const hackathon = await hackathonPlatformCore.getHackathon(hackathonId);
    ...
    it("Should track multiple sponsors", async function () {
      ...
      await hackathonPlatformCore
        .connect(sponsor2)
        .sponsorHackathon(hackathonId, "ipfs://sponsor2", {
          value: SPONSOR_AMOUNT,
        });

      const sponsors = await hackathonPlatformCore.getSponsors(hackathonId);
  ...
  describe("5. Team Management", function () {
    async function verifyGitHub(user, githubId, username) {
      ...
      await githubVerifier
        .connect(user)
        .verifyGitHub(githubId, username, nonce, timestamp, signature);
    }
    ...
    it("Should create a team", async function () {
      await expect(
        hackathonPlatformCore.connect(user1).createTeam("ipfs://team1", "secretcode123")
      )
        .to.emit(hackathonPlatformCore, "TeamCreated")
        .withArgs(0, user1.address);

      const team = await hackathonPlatformCore.getTeam(0);
    ...
    it("Should reject team creation without GitHub verification", async function () {
      await expect(
        hackathonPlatformCore.connect(user4).createTeam("ipfs://team1", "secretcode123")
      ).to.be.revertedWith("GitHub not verified");
    });

    it("Should reject team creation with short join code", async function () {
      await expect(
        hackathonPlatformCore.connect(user1).createTeam("ipfs://team1", "short")
    ...
    it("Should allow joining team with correct code", async function () {
      await hackathonPlatformCore
        .connect(user1)
        .createTeam("ipfs://team1", "secretcode123");

      await expect(hackathonPlatformCore.connect(user2).joinTeam(0, "secretcode123"))
        .to.emit(hackathonPlatformCore, "TeamMemberAdded")
    ...
    it("Should reject joining team with wrong code", async function () {
      ...
      await expect(
        hackathonPlatformCore.connect(user2).joinTeam(0, "wrongcode")
    ...
    it("Should reject duplicate team members", async function () {
      ...
      await expect(
        hackathonPlatformCore.connect(user1).joinTeam(0, "secretcode123")
  ...
  describe("6. Team Registration & Staking", function () {
    ...
    beforeEach(async function () {
      ...
      await hackathonPlatformCore
        .connect(user1)
        .createTeam("ipfs://team1", "secretcode123");
      await hackathonPlatformCore.connect(user2).joinTeam(0, "secretcode123");
      teamId = 0;
    ...
    it("Should register team for hackathon", async function () {
      await expect(hackathonPlatformCore.connect(user1).registerTeam(hackathonId, teamId))
        .to.emit(hackathonPlatformCore, "TeamRegistered")
        .withArgs(hackathonId, teamId);

      const registration = await hackathonPlatformCore.getTeamRegistration(
        hackathonId,
        teamId
      );
    ...
    it("Should reject registration during sponsorship phase", async function () {
      ...
      await expect(
        hackathonPlatformCore.connect(user1).registerTeam(1, teamId)
      ).to.be.revertedWith("Sponsorship phase not ended");
    });

    it("Should stake for team", async function () {
      await hackathonPlatformCore.connect(user1).registerTeam(hackathonId, teamId);

      await expect(
        hackathonPlatformCore
          .connect(user1)
          .stakeForTeam(hackathonId, teamId, { value: STAKE_AMOUNT })
      )
        .to.emit(hackathonPlatformCore, "TeamStaked")
    ...
    it("Should mint voting tokens after staking", async function () {
      ...
      const tokenAddress = await hackathonPlatformCore.getVotingToken(hackathonId);
      const votingToken = await ethers.getContractAt(
        "HackathonVotingToken",
        tokenAddress
      );

      const tokensPerParticipant = ethers.parseEther("100");
      expect(await votingToken.balanceOf(user1.address)).to.equal(
        tokensPerParticipant
      );
      expect(await votingToken.balanceOf(user2.address)).to.equal(
        tokensPerParticipant
      );
    });
    ...
    it("Should reject staking with incorrect amount", async function () {
      ...
      await expect(
        hackathonPlatformCore
          .connect(user1)
          .stakeForTeam(hackathonId, teamId, {
  ...
  describe("7. Complete Workflow Test", function () {
    ...
    beforeEach(async function () {
      ...
      // Create teams
      await hackathonPlatformCore.connect(user1).createTeam("ipfs://team1", "secretcode1");
      team1Id = 0;
      await hackathonPlatformCore.connect(user2).joinTeam(team1Id, "secretcode1");

      await hackathonPlatformCore.connect(user3).createTeam("ipfs://team2", "secretcode2");
      team2Id = 1;
      await hackathonPlatformCore.connect(user4).joinTeam(team2Id, "secretcode2");

      await hackathonPlatformCore.connect(user5).createTeam("ipfs://team3", "secretcode3");
      team3Id = 2;
      await hackathonPlatformCore.connect(user6).joinTeam(team3Id, "secretcode3");

      // Register teams
      await hackathonPlatformCore.connect(user1).registerTeam(hackathonId, team1Id);
      await hackathonPlatformCore.connect(user3).registerTeam(hackathonId, team2Id);
      await hackathonPlatformCore.connect(user5).registerTeam(hackathonId, team3Id);

      // Stake
      await hackathonPlatformCore
        .connect(user1)
        .stakeForTeam(hackathonId, team1Id, { value: STAKE_AMOUNT });
      await hackathonPlatformCore
        .connect(user3)
        .stakeForTeam(hackathonId, team2Id, { value: STAKE_AMOUNT });
      await hackathonPlatformCore
        .connect(user5)
        .stakeForTeam(hackathonId, team3Id, { value: STAKE_AMOUNT });
      ...
      await hackathonPlatformCore
        .connect(user1)
        .submitProject(hackathonId, team1Id, ethers.id("repo1"), 85);
      await hackathonPlatformCore
        .connect(user3)
        .submitProject(hackathonId, team2Id, ethers.id("repo2"), 90);
      await hackathonPlatformCore
        .connect(user5)
        .submitProject(hackathonId, team3Id, ethers.id("repo3"), 75);
    ...
    it("Should complete full judging and scoring workflow", async function () {
      ...
      await hackathonPlatformCore
        .connect(judge1)
        .submitJudgeScore(hackathonId, team1Id, 80);
      await hackathonPlatformCore
        .connect(judge2)
        .submitJudgeScore(hackathonId, team1Id, 85);
      await hackathonPlatformCore
        .connect(judge1)
        .submitJudgeScore(hackathonId, team2Id, 95);
      await hackathonPlatformCore
        .connect(judge2)
        .submitJudgeScore(hackathonId, team2Id, 90);
      await hackathonPlatformCore
        .connect(judge1)
        .submitJudgeScore(hackathonId, team3Id, 70);
      ...
      await hackathonPlatformCore
        .connect(user1)
        .voteForTeam(hackathonId, team2Id, ethers.parseEther("50"));
      await hackathonPlatformCore
        .connect(user2)
        .voteForTeam(hackathonId, team2Id, ethers.parseEther("50"));
      await hackathonPlatformCore
        .connect(user3)
        .voteForTeam(hackathonId, team1Id, ethers.parseEther("50"));

      // Calculate final scores
      await hackathonPlatformCore.calculateFinalScores(hackathonId);

      const team1Reg = await hackathonPlatformCore.getTeamRegistration(
        hackathonId,
        team1Id
      );
      const team2Reg = await hackathonPlatformCore.getTeamRegistration(
        hackathonId,
        team2Id
      );
    ...
    it("Should distribute rewards correctly", async function () {
      ...
      // Distribute rewards
      await hackathonPlatformCore.distributeRewards(hackathonId);
    ...
    it("Should refund stakes", async function () {
      ...
      const tx = await hackathonPlatformCore
        .connect(user1)
        .refundStake(hackathonId, team1Id);
