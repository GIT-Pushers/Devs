...
const sampleHackathons: Hack[] = [
  {
    ...
    register: [{ type: 'GitHub', href: '#' }, { type: 'Google Form', href: '#' }],
    featured: true,
    location: 'Online',
  },
  {
    ...
    tags: ['blockchain', 'eth'],
    register: [{ type: 'Repo Link', href: '#' }, { type: 'Direct Upload', href: '#' }],
    featured: false,
    location: 'Hybrid',
  },
  {
    ...
    register: [{ type: 'GitHub', href: '#' }, { type: 'Slack Signup', href: '#' }],
    featured: false,
    location: 'Online',
  },
]
...
function Badge({ children }: { children: React.ReactNode }) {
  return (
    <span className="px-2 py-1 text-xs font-medium rounded-full border border-black/10 bg-black/5">{children}</span>
  )
}
...
export default function Home() {
  ...
  const [sort, setSort] = useState<'upcoming' | 'newest' | 'featured'>('upcoming')
  ...
  const filtered = useMemo(() => {
    ...
    let list = sampleHackathons.filter((h) => {
      ...
      if (!q) return true
      return [h.title, h.desc, h.tags.join(' ')].join(' ').toLowerCase().includes(q)
    })
      ...
      <section className="container mx-auto px-6 py-8">
        ...
        <div className="max-w-3xl mx-auto">
          <div className="relative">
            <div className="flex items-center gap-3 border rounded-full px-4 py-2 shadow-sm bg-white">
              ...
              />
              <select value={tag} onChange={(e) => setTag(e.target.value)} className="border-l pl-3 text-sm">
                {tags.map((t) => (
        ...
        <div className="mt-10">
          <div className="grid grid-cols-1 gap-8">
            <div>
            <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight">Find hackathons, join communities, ship projects.</h1>
            <p className="mt-4 text-gray-700 max-w-2xl">HackX is a curated home for hackathons and developer communities — discover upcoming events, submit repos for automated checks, and connect with mentors.</p>

            <div className="mt-6 flex gap-3 flex-wrap">
              <a href="#hackathons" className="inline-flex items-center gap-2 px-5 py-3 btn-primary font-medium">Explore Hackathons</a>
              <a href="#community" className="inline-flex items-center gap-2 px-5 py-3 btn-secondary font-medium">Community</a>
            </div>

            <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
              <Card title="Automated Checks" desc="Commit history, contributor checks and plagiarism scanning." />
              <Card title="Mentors & Prizes" desc="Find mentors and prize tracks tailored to your tech stack." />
              <Card title="Community" desc="Active Discords, Meetups and open office hours." />
            </div>
          </div>
          </div>
        </div>
        ...
        <div id="hackathons" className="mt-12">
          <div className="flex items-center justify-between">
            ...
            <div className="text-sm text-gray-600">Curated — updated weekly</div>
          </div>
    ...
    <article className="p-6 border rounded-xl bg-white shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <div className="text-sm text-gray-600">{h.dates} · {h.location}</div>
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {h.register.map((r, idx) => (
          <a key={idx} href={r.href} className="px-3 py-2 border rounded-md text-sm transition btn-secondary">Register ({r.type})</a>
        ))}
      </div>
    </article>
  )
...
function CommunityCard({ name, members, activity }: { name: string; members: number; activity: string }) {
  return (
    <div className="p-4 border rounded-xl bg-white">
      <div className="flex items-center justify-between">
        <div>
          <div className="font-medium">{name}</div>
          <div className="text-xs text-gray-500">{activity} · {members.toLocaleString()} members</div>
        </div>
        <div className="text-sm">
          <a href="#" className="px-3 py-1 border rounded">Join</a>
        </div>
      </div>
    </div>
  )
