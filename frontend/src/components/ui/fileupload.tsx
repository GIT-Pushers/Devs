...
export const FileUploader: React.FC<FileUploaderProps> = ({ label, onChange, error, value }) => {
    ...
    <div className="w-full">
      ...
      {!preview ? (
        ...
        >
          ...
          <p className="text-sm font-medium text-neutral-900">Click to upload team image</p>
        ...
        <div className="relative rounded-lg overflow-hidden border border-neutral-200 bg-neutral-50">
          ...
          <div className="absolute top-2 right-2">
             <button
              type="button"
              onClick={clearFile}
              className="p-1.5 bg-white/90 backdrop-blur-sm rounded-full shadow-sm hover:bg-red-50 text-neutral-600 hover:text-red-600 transition-colors"
             >
               <X className="w-4 h-4" />
             </button>
          </div>
          <div className="px-4 py-2 bg-white border-t border-neutral-100 flex items-center gap-2">
             ...
             <span className="text-sm text-neutral-700 truncate">{value?.name}</span>
          </div>
        </div>
      )}
