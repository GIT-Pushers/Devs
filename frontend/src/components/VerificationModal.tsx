...
export function VerificationModal({ isOpen, onClose, githubUser }: VerificationModalProps) {
  ...
  const prepareVerification = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to prepare verification');
      }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 overflow-y-auto" onClick={onClose}>
      <div className="bg-card border border-border rounded-lg p-6 max-w-md w-full shadow-lg my-8" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-start mb-6">
          <div className="flex-1 pr-4">
            <h2 className="text-xl font-semibold mb-2">Verify Your GitHub Account</h2>
            <p className="text-sm text-muted-foreground">
              Link your GitHub account to your wallet to participate in hackathons
            </p>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-muted p-4 rounded-lg">
              <p className="text-sm mb-2">
                <span className="font-semibold">GitHub:</span> {githubUser?.login || githubUser?.name}
              </p>
              <p className="text-sm">
                <span className="font-semibold">Wallet:</span> {account?.address.slice(0, 6)}...{account?.address.slice(-4)}
              </p>
            </div>
            <p className="text-sm text-muted-foreground">
