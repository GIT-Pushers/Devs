...
export const ProjectSubmissionForm: React.FC = () => {
    ...
    const [status, setStatus] = useState<'idle' | 'analyzing' | 'submitting' | 'success' | 'error'>('idle');
    const [result, setResult] = useState<AnalysisResult | null>(null);

    const validate = (): boolean => {
        const newErrors: typeof errors = {};
        if (!formData.hackathonId) newErrors.hackathonId = 'Hackathon ID is required';
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const mockResult = {
            repoHash: '0x' + Array.from({ length: 40 }, () => Math.floor(Math.random() * 16).toString(16)).join(''),
    ...
    if (status === 'success' && result) {
        return (
            <div className="bg-white p-8 rounded-xl border border-neutral-200 shadow-sm text-center max-w-lg mx-auto animate-fadeIn">
                ...
                <h3 className="text-2xl font-bold text-neutral-900 mb-2">Project Submitted!</h3>
                <p className="text-neutral-600 mb-6">
                    Your project has been successfully analyzed and submitted to the blockchain.
                </p>

                <div className="bg-neutral-50 rounded-lg p-4 text-left space-y-3 mb-8 border border-neutral-100">
                    <div className="flex justify-between items-center">
                        <span className="text-sm font-medium text-neutral-500">AI Quality Score</span>
                        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-bold bg-indigo-100 text-indigo-800">
                            {result.aiScore}/100
                        </span>
                    </div>
                    <div className="border-t border-neutral-200 my-2"></div>
                    <div>
                        <span className="text-xs font-medium text-neutral-500 block mb-1">Repo Hash</span>
    ...
    return (
        <div className="bg-white p-6 md:p-10 rounded-2xl border border-neutral-200 shadow-sm max-w-2xl mx-auto">
            <div className="mb-8">
                ...
                <p className="text-neutral-500 mt-1">Submit your hackathon project for AI analysis and verification.</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label className="block text-sm font-medium text-neutral-900 mb-2">Hackathon ID</label>
                        <Input
                            type="number"
                            placeholder="e.g. 1"
                        ...
                        />
                        {errors.hackathonId && <p className="text-red-600 text-sm mt-1">{errors.hackathonId}</p>}
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-neutral-900 mb-2">Team ID</label>
                        <Input
                            type="number"
                            placeholder="e.g. 42"
                        ...
                        />
                        {errors.teamId && <p className="text-red-600 text-sm mt-1">{errors.teamId}</p>}
                    </div>
                </div>

                <div>
                    <label className="block text-sm font-medium text-neutral-900 mb-2">GitHub Repository URL</label>
                    ...
                    {errors.repoUrl && <p className="text-red-600 text-sm mt-1">{errors.repoUrl}</p>}
                    <p className="text-xs text-neutral-500 mt-2">
                        Make sure your repository is public for the AI analysis tool to access it.
                    </p>
                </div>

                {status === 'analyzing' && (
                    <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-lg flex items-center gap-3 text-indigo-700 animate-pulse">
